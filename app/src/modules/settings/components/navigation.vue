<template>
	<v-list nav>
		...
		<v-list-item href="https://github.com/pxslip/db-studio/releases" class="version">
			<v-list-item-icon><v-icon name="directus" /></v-list-item-icon>
			<v-list-item-content>
				<v-text-overflow class="version" :text="`Version ${version}`" />
			</v-list-item-content>
		</v-list-item>
	</v-list>
</template>
			...
			return [
				{
					icon: 'bug_report',
					name: t('report_bug'),
					href: 'https://github.com/pxslip/db-studio/issues/new?template=bug_report.yml',
				},
				{
					icon: 'new_releases',
					name: t('request_feature'),
					href: 'https://github.com/pxslip/db-studio/discussions/new?category=feature-requests',
				},
			];
		});
