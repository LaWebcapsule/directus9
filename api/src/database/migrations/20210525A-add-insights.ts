...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('directus9_dashboards', (table) => {
	...
	await knex.schema.createTable('directus9_panels', (table) => {
		table.uuid('id').primary().notNullable();
		table.uuid('dashboard').notNullable().references('id').inTable('directus9_dashboards').onDelete('CASCADE');
		table.string('name');
...
export async function down(knex: Knex): Promise<void> {
	await knex.schema.dropTable('directus9_panels');
	await knex.schema.dropTable('directus9_dashboards');
}
