...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('directus9_relations').update({ one_deselect_action: 'nullify' });

	await helper.changeToType('directus9_relations', 'sort_field', 'string', {
		length: 64,
	});

	await helper.changeToType('directus9_relations', 'one_deselect_action', 'string', {
		nullable: false,
		default: 'nullify',
	});
}
