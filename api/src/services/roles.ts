...
export class RolesService extends ItemsService {
	...
	private async checkForOtherAdminUsers(key: PrimaryKey, users: Alterations | Item[]): Promise<void> {
		const role = await this.knex.select('admin_access').from('directus9_roles').where('id', '=', key).first();
		...
		const usersThatWereInRoleBefore = (await this.knex.select('id').from('directus9_users').where('role', '=', key)).map(
			(user) => user.id
		);
		...
		const otherAdminUsers = await this.knex
			...
			.whereNotIn('directus9_users.id', [...userKeys, ...usersThatAreRemoved])
	...
	override async deleteMany(keys: PrimaryKey[]): Promise<PrimaryKey[]> {
		...
		await this.knex.transaction(async (trx) => {
			const itemsService = new ItemsService('directus9_roles', {
				knex: trx,
