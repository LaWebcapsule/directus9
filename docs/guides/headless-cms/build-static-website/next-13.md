---
description: This guide shows you how build a website with Next 13 and Directus9 as a Headless CMS.
tags: []
skill_level:
...
# Build a Website With Next 13 and the Directus9 JavaScript SDK
...
To share a single instance of the Directus9 JavaScript SDK between multiple pages in this project, create a single helper
file that can be imported later. Create a new directory called `lib` and a new file called `directus9.js` inside of it.

```js
import { Directus9 } from '@directus9/sdk';
const directus9 = new Directus9('https://your-project-id.directus9.app/');
export default directus9;
```
...
![A form named "Global" has two inputs - a title and a description, each filled with some text.](https://cdn.directus9.io/docs/v9/headless-cms/how-to-packet-20220222A/next-global-config.webp)
...
```js
import directus9 from 'lib/directus9';

async function getGlobals() {
	const { data } = await directus9.items('global').readByQuery();
	return data;
}
...
browser. You should see data from your Directus9 Global collection in your page. Some additional files will be created by
...
[here's some sample data](https://github.com/directus9/examples/blob/main/website-next13/demo-data).
...
async function getPage(slug) {
	try {
		const page = await directus9.items('pages').readOne(slug);
		return page;
	} catch (error) {
		notFound();
	}
...
In Roles & Permissions, give the Public role read access to the `authors`, `posts`, and `directus9_files` collections.
...
async function getPosts() {
	const posts = await directus9.items('posts').readByQuery({
...
![A page with a title of "Blog". On it is a list of three items - each with a title, author, and date. The title is a link.](https://cdn.directus9.io/docs/v9/headless-cms/how-to-packet-20220222A/next-blog-listing.webp)
...
async function getPost(slug) {
	try {
		const post = await directus9.items('posts').readOne(slug, {
			fields: ['*.*'],
		});
		return post;
	} catch (error) {
...
![A blog post page shows an image, a title, and a number of paragraphs.](https://cdn.directus9.io/docs/v9/headless-cms/how-to-packet-20220222A/next-blog-single.webp)

## Add Navigation

While not strictly Directus9-related, there are now several pages that aren't linked to each other. In `app/layout.tsx`,
...
Through this guide, you have set up a Next project, created a Directus9 helper, and used it to query data. You have used
...
[on GitHub](https://github.com/directus9/examples/blob/main/website-next13).
